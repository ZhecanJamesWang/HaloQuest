    ...
    def forward(self, samples):
        ...
        with self.maybe_autocast(dtype=torch.float):
            ...
            for b, n in enumerate(samples['n_answers']):
                question_states += [inputs_embeds[b]]*n
                question_atts += [encoder_atts[b]]*n                
            question_states = torch.stack(question_states,0)    
            question_atts = torch.stack(question_atts,0)     

            outputs = self.t5_model(
                inputs_embeds=question_states,
            ...
            loss = outputs.loss
            loss = samples["weight"] * loss
            bsz = samples["image"].size(0)
            loss = loss.sum() / bsz
            # mean_loss = weighted_loss[targets!=-100].sum()/(targets!=-100).sum()
            return {"loss": loss}
